...
export const ScriptNode = ({ data, selected }: any) => {
    ...
    <div className={`relative w-72 bg-white rounded-2xl overflow-hidden transition-all duration-300 group ${ringClass}`}>
      ...
      <div className={`h-10 px-4 flex items-center justify-between transition-colors ${headerClass}`}>
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] font-bold uppercase tracking-widest text-stone-500">Compute</span>
        </div>
        {/* <MaterialIcon icon="terminal" className="text-sm text-stone-400" /> */}
      </div>
      <div className="p-5 flex items-center gap-4">
        ...
        <div className="flex-1">
          <h3 className="text-sm font-semibold text-stone-800">Post-Process</h3>
          <p className="text-xs text-stone-500">Transforming payload...</p>
        </div>
        <div className={`w-5 h-5 rounded-full border-2 border-blue-600 border-t-transparent animate-spin transition-opacity ${status === 'running' ? 'opacity-100' : 'opacity-0'}`}></div>
      </div>
      {status === 'failed' && (
        <div className="mx-4 mb-4 px-3 py-2 rounded-lg bg-red-50 border border-red-100 text-red-700 text-[11px] leading-tight flex gap-2">
           <span className="font-semibold text-red-800">Error:</span> {data?.error || "ReferenceError: payload is not defined"}
        </div>
      )}
